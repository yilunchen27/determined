...
def pytest_addoption(parser: Parser) -> None:
    ...
    path = Path(__file__).parents[2].joinpath("deploy", "determined_deploy", "local", "docker-compose.yaml")
    parser.addoption(
        "--compose-file", action="store", default=str(path), help="Docker compose file"
    )
    parser.addoption(
        "--compose-project-name", action="store", default="determined", help="Docker compose project name"
    )
